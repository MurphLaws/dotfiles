...
bool waste_pile_stack(struct stack *stack) {
  return ((stack->card->frame->begin_y == WASTE_PILE_BEGIN_Y) &&
          (stack->card->frame->begin_x == WASTE_PILE_BEGIN_X + game.x_offset));
}

static bool foundation_stack(struct stack *stack) {
  int off = game.x_offset;
  return (stack->card->frame->begin_y == FOUNDATION_BEGIN_Y &&
          (stack->card->frame->begin_x == FOUNDATION_0_BEGIN_X + off ||
           stack->card->frame->begin_x == FOUNDATION_1_BEGIN_X + off ||
           stack->card->frame->begin_x == FOUNDATION_2_BEGIN_X + off ||
           stack->card->frame->begin_x == FOUNDATION_3_BEGIN_X + off));
}

bool stock_stack(struct stack *stack) {
  return ((stack->card->frame->begin_y == STOCK_BEGIN_Y) &&
          (stack->card->frame->begin_x == STOCK_BEGIN_X + game.x_offset));
}

bool maneuvre_stack(struct stack *stack) {
  int off = game.x_offset;
  return (stack->card->frame->begin_y >= MANEUVRE_BEGIN_Y &&
          (stack->card->frame->begin_x == MANEUVRE_0_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_1_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_2_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_3_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_4_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_5_BEGIN_X + off ||
           stack->card->frame->begin_x == MANEUVRE_6_BEGIN_X + off));
}
...
static void fill_deck(struct deck *deck) {
  ...
  /* Use the stock placeholder's (already-offset) position so every card
   * sitting on the stock appears at the centered location. */
  int begin_y = deck->stock->card->frame->begin_y;
  int begin_x = deck->stock->card->frame->begin_x;

  for (int i = ACE; i <= KING; i++) {
    for (int j = DIAMONDS; j <= CLUBS; j++) {
      int index = 4 * (i - ACE) + j;
      card_malloc(&(card[index]));
      card_set(card[index], i, j, COVERED, begin_y, begin_x);
...
void game_init(struct game *game, int passes_through_deck,
               int four_color_deck) {
  /* Compute horizontal offset to center the board. */
  int screen_rows, screen_cols;
  getmaxyx(stdscr, screen_rows, screen_cols);
  (void)screen_rows;
  game->x_offset = (screen_cols - 56) / 2;
  if (game->x_offset < 0)
    game->x_offset = 0;
