...
pub enum Arg {
    ...
    GridSize(Size),
    /// Launch the Dungeons & Diagrams variant with the given grid size.
    Dnd(Size),
    Help,
    Version,
}
...
pub fn parse() -> Result<Option<Arg>, Cow<'static, str>> {
    ...
    if let Some(arg) = args.next() {
        if let Ok(first_string) = arg.into_string() {
            // `dnd` subcommand: Dungeons & Diagrams variant.
            if first_string == "dnd" {
                let second = args.next().and_then(|a| a.into_string().ok());
                let third = args.next().and_then(|a| a.into_string().ok());
                return parse_dnd(second.as_deref(), third.as_deref()).map(Some);
            }
...
/// Parse `yayagram dnd [size|width height]`. Defaults to 8×8.
fn parse_dnd(
    second: Option<&str>,
    third: Option<&str>,
) -> Result<Arg, Cow<'static, str>> {
    let size = match (second, third) {
        (None, _) => Size {
            width: 8,
            height: 8,
        },
        (Some(w), Some(h)) => match parse_size(w, h) {
            Ok(Some(Arg::GridSize(size))) => size,
            Ok(_) => return Err("Invalid dnd size".into()),
            ...
            Err(SizeError::FileNotFound) => {
                return Err("Invalid dnd size".into())
            }
        },
        (Some(s), None) => match parse_squared_size(s) {
            Ok(Some(Arg::GridSize(size))) => size,
            Ok(_) => return Err("Invalid dnd size".into()),
            Err(SizeError::OutOfRange(thing)) => {
                return Err(format!(
