use super::{Cell, Grid, PuzzleMode};
...
impl Builder {
    ...
    fn draw_top_clues(&mut self, terminal: &mut Terminal) -> usize {
        ...
        for (x, vertical_clues_solution) in self.grid.vertical_clues_solutions.iter().enumerate() {
            let solved = match self.grid.mode {
                PuzzleMode::Nonogram => {
                    let vertical_clues = self.grid.get_vertical_clues(x as u16);
                    vertical_clues.eq(vertical_clues_solution.iter().copied())
                }
                PuzzleMode::DungeonsAndDiagrams => {
                    let target = vertical_clues_solution.first().copied().unwrap_or(0);
                    self.grid.col_walls(x as u16) == target
                }
            };
            ...
            self.grid.horizontal_clues_solutions.iter().enumerate()
        {
            ...
            let solved = match self.grid.mode {
                PuzzleMode::Nonogram => {
                    ...
                    horizontal_clues.eq(horizontal_clues_solution.iter().copied())
                }
                PuzzleMode::DungeonsAndDiagrams => {
                    let target = horizontal_clues_solution.first().copied().unwrap_or(0);
                    self.grid.row_walls(y as u16) == target
                }
            };
    ...
    pub fn draw_all(&mut self, terminal: &mut Terminal) -> bool {
        ...
        match self.grid.mode {
            ...
            PuzzleMode::DungeonsAndDiagrams => self.grid.dnd_is_solved(),
        }
    }
}
