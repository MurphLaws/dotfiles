...
pub enum Cell {
    ...
    /// Dungeons & Diagrams: pre-placed monster hint (sits in a dead end).
    Monster,
    /// Dungeons & Diagrams: pre-placed treasure hint (inside a 3×3 treasure room).
    Treasure,
}

impl Cell {
    pub fn is_hint(&self) -> bool {
        matches!(self, Cell::Monster | Cell::Treasure)
    }
}
...
impl Cell {
    pub fn get_color(&self) -> Color {
        match self {
            ...
            // Hint cells render on an empty-coloured background; the glyph is
            // what carries the color. Returning default() lets the Empty
            // checkerboard background logic run in `draw`.
            Cell::Monster | Cell::Treasure => Color::default(),
        }
    }
    ...
    fn get_highlighted_color(&self) -> Color {
        match self {
            ...
            Cell::Monster | Cell::Treasure => Color::DarkGray,
        }
    }
    ...
    pub fn draw(&self, terminal: &mut Terminal, point: Point, highlight: bool) {
        ...
        let (foreground_color, background_color, content) = match self {
            Cell::Empty => {
                background_color = checkerboard_background(highlight);
            ...
            Cell::Monster => {
                background_color = checkerboard_background(highlight);
                (Some(Color::Red), background_color, "☠ ".into())
            }
            Cell::Treasure => {
                background_color = checkerboard_background(highlight);
                (Some(Color::Yellow), background_color, "◆ ".into())
            }
...
impl CellPlacement {
    pub fn place(
        ...
        // D&D hint cells are pre-placed and must not be modified by the player.
        if grid_cell.is_hint() {
