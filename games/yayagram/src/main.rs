    ...
    let grid = match arg {
        Ok(Some(args::Arg::Help)) => {
            println!(concat!(
                ...
                "\n",
                "Usage:\n",
                "    yayagram                  Play a random nonogram (5x5).\n",
                "    yayagram <size>           Play a random nonogram of the given size.\n",
                "    yayagram <width> <height> Play a random nonogram with the given dimensions.\n",
                "    yayagram <file.yaya>      Load a grid from a .yaya file.\n",
                "    yayagram dnd              Play a random Dungeons & Diagrams puzzle (8x8).\n",
                "    yayagram dnd <size>       Play a random Dungeons & Diagrams puzzle of that size.\n",
                "    yayagram dnd <w> <h>      Play a random Dungeons & Diagrams puzzle with the given dimensions."
            ));
...
fn get_grid(arg: Result<Option<args::Arg>, Cow<'static, str>>) -> Result<Grid, Cow<'static, str>> {
    match arg {
        Ok(arg) => match arg {
            ...
            Some(args::Arg::Dnd(size)) => Ok(Grid::dnd(size)),
            arg => {
