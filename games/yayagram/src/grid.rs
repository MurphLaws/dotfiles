...
pub type Clues = Vec<Clue>;

/// Which puzzle ruleset the grid is using.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PuzzleMode {
    /// Classic nonogram: clues are run-lengths of filled cells.
    Nonogram,
    /// Dungeons & Diagrams: clues are total wall counts per row/column, and
    /// hint cells (Monster/Treasure) are pre-placed on the board.
    DungeonsAndDiagrams,
}
...
pub struct Grid {
    ...
    pub mode: PuzzleMode,
    /// For D&D mode, the full reference solution (walls + hint cells). Used to
    /// determine whether the player's shading matches the intended dungeon.
    pub solution_cells: Option<Vec<Cell>>,
}

pub fn get_index(grid_width: u16, point: Point) -> usize {
...
impl Grid {
    ...
    pub fn new(size: Size, mut cells: Vec<Cell>) -> Self {
        ...
        Self {
            ...
            undo_redo_buffer,
            measurement_counter,
            mode: PuzzleMode::Nonogram,
            solution_cells: None,
        }
    }

    /// Returns the number of filled (wall) cells in row `y`. Used by D&D-mode
    /// clue rendering and per-row solved highlighting.
    pub fn row_walls(&self, y: u16) -> u16 {
        (0..self.size.width)
            .filter(|x| self.get_cell(Point { x: *x, y }) == Cell::Filled)
            .count() as u16
    }

    /// Returns the number of filled (wall) cells in column `x`.
    pub fn col_walls(&self, x: u16) -> u16 {
        (0..self.size.height)
            .filter(|y| self.get_cell(Point { x, y: *y }) == Cell::Filled)
            .count() as u16
    }

    /// D&D solve check: the player wins when every non-hint cell matches the
    /// reference solution's filled/empty state. (Hint cells are never modified.)
    pub fn dnd_is_solved(&self) -> bool {
        let Some(solution) = self.solution_cells.as_ref() else {
            return false;
        };
        self.cells
            .iter()
            .zip(solution.iter())
            .all(|(player, sol)| match sol {
                Cell::Monster | Cell::Treasure => true,
                Cell::Filled => *player == Cell::Filled,
                _ => *player != Cell::Filled,
            })
    }
