...
**First check for a text layer** — a scanned book bypasses this whole pipeline:

```bash
pdftotext -f 1 -l 5 input.pdf - | wc -c   # ~0 bytes → scanned PDF
```

If scanned, follow [reference/scanned-pdfs.md](reference/scanned-pdfs.md)
(tesseract OCR, spread splitting, typo verification, popup footnotes). Do not
OCR whole books with LLM vision — it hallucinates; details in that file.
...
│   └─► pdftotext returns nothing → reference/scanned-pdfs.md
│       (tesseract pipeline, EPUB typo detection, popup footnotes,
│        section-inventory check against the EPUB spine)
│
└─► Other issue?
...
Long conversions (especially scanned PDFs) run for many minutes. Keep the user
oriented the whole way:

- **At start**, state the phase plan with weights and a total time estimate,
  e.g.: analyze 5% · OCR 35% · verification 15% · typo fixes 10% ·
  footnotes 25% · packaging 5% · final metric 5% — "~40 min total".
- **After each phase** (and at least every few minutes inside a long phase),
  post one line with THREE numbers — progress, time left, and the fidelity
  measured so far:
  `Fase erratas lista — 60% del proceso, ~12 min restantes, fidelidad actual 97,6% → objetivo >99%.`
- **Measure fidelity incrementally**: run a preliminary word-level ratio
  (difflib, normalized words) against the source right after OCR/extraction,
  then recompute after each correction phase (typos, footnotes, cleanup) so
  the user sees the number climb toward the target.
- **Do NOT narrate internal mistakes or corrections** ("I was wrong, fixing
  it..."). Verify silently, iterate silently; self-correction is part of the
  work, not part of the report. Surface only consolidated status and findings
  that change the outcome.
- **At the end**, deliver: the generated file path, the measured fidelity
  metric, what was corrected, and honest limitations.
...
- [Scanned PDFs](reference/scanned-pdfs.md) - Image-only PDFs: tesseract pipeline, verification, popup footnotes, notes-parser validation, tables/indexes as images, known failure catalog
