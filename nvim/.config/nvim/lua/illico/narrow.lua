...
-- buffer temporal a PANTALLA COMPLETA (pestaña nueva). Al guardar (:w) en ese
-- buffer, los cambios se escriben de vuelta en el buffer .md original (que queda
-- modificado, listo para guardarlo). Cierra la pestaña con :q / :wq.
local M = {}
...
-- Abre las líneas [s,e] de `srcbuf` en un buffer temporal en una pestaña nueva.
function M.narrow(srcbuf, s, e)
	...
	-- Pestaña nueva (pantalla completa) y mostrar ahí el buffer temporal.
	vim.cmd("tabnew")
